...
| Version | Name | Status |
|---|---|---|
| 20260922200100 | public_listings_description | applied 2026-09-22 |
| 20260922300100 | homepage_media_and_featured | **pending** — paste `supabase/migrations/20260922300100_homepage_media_and_featured.sql`, then `insert into supabase_migrations.schema_migrations (version, name) values ('20260922300100', 'homepage_media_and_featured');` |

Until the pending one is applied, production `/` fails (no `homepage_media` table / `featured_at` column there). Do not push `main` before running it.
